 ...
 */
class Plugin_Tnavigation extends Plugin
{

	public $version = '1.0.0';
	public $name = array(
		'en' => 'Translated Navigation',
	);
	public $description = array(
		'en' => 'Translated navigation links including links in dropdown menus.',
	);
	...
	public function links()
	{
		...
		$translate     = $this->attribute('translate', TRUE);
		$match_segment     = $this->attribute('match_segment', false);
	 ...
	 * @return array|string
	 */
	private function _build_links($links = array(), $return_arr = true, $match_segment = null)
	{
		...
		$match_segment_string = null;
		if ($match_segment)
		{
			$match_segment = (int) $match_segment;
			$match_segment_string = '';
			for ($n = 1; $n <= $match_segment; $n++)
			{
				$match_segment_string .= $this->uri->segment($n).DIRECTORY_SEPARATOR;
			}
			$match_segment_string = rtrim($match_segment_string, DIRECTORY_SEPARATOR);
		}
		...
		foreach ($links as $link)
		{
			...
			if ( $link['children'] )
			{
				...
				if ( ! $max_depth or $level < $max_depth )
				{
					...
					$wrapper['children'] = $this->_build_links($link['children'], $return_arr, ++$match_segment);
				}
			...
			elseif ($match_segment and $this->uri->segment(1) != '')
			{

				if (strpos($link['url'], site_url().$match_segment_string) !== false)
				{
					//var_dump($link['children']);
					$current_link = $link['url'];
					$wrapper['class'][] = $current_class;
				}
			}
